...
export APP=story-api
export APP_HOME=/home/story/story-api
...
if [ "${RES_ENV}" = "true" ]; then
    ...
    export ENV
    export JWT_SECRET=$(aws secretsmanager get-secret-value --secret-id "$ENV/story-api/jwt" --region=$REGION --query SecretString --output text)
    DBKEY=
    case "$ENV_TAG_VALUE" in
    development)
